...
def _add_member(session: Session, circle: Circle, user: User) -> Membership:
    ...
    if len(circle_members(session, circle.id)) >= circle.member_limit:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
def list_circles(
    ...
    if current_user.role == UserRole.ADMIN:
        circles = session.exec(
            select(Circle).where(Circle.admin_id == current_user.id)
        ).all()
    else:
        member_rows = session.exec(
            select(Membership).where(Membership.user_id == current_user.id)
        ).all()
        ids = [row.circle_id for row in member_rows]
        if not ids:
            return []
        circles = session.exec(select(Circle).where(Circle.id.in_(ids))).all()
...
def set_turn_order(
    ...
    # Unique (circle, turn_position) will fight a swap of 1 and 2,
    # so we park everyone high first, then write the real order.
