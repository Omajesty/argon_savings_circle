...
def get_all_circles(
    ...
    circles = session.exec(select(Circle)).all()
...
def join_circle(
    ...
    circle = get_circle_or_404(session, circle_id)
    if get_membership(session, current_user.id, circle.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Already a member of this circle",
        )
    ...
    existing = session.exec(
        select(MembershipRequest).where(
            ...
            MembershipRequest.circle_id == circle.id,
    ...
    if existing:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="You already have a pending request for this circle",
        )
